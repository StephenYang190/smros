...
using SCPointType =
        SemanticSurfel; // using xyz only. but a user can exchange the original bin encoding
// function (i.e., max hegiht) to max intensity (for detail, refer
// 20 ICRA Intensity Scan Context)
...
public:
    ...
    Eigen::MatrixXd makeScancontext(pcl::PointCloud<SemanticSurfel> &_scan_down);
    ...
    // User-side API
    void makeAndSaveScancontextAndKeys(pcl::PointCloud<SemanticSurfel> &_scan_down);
