...
std::pair<double, int> SCManager::distanceBtnScanContext(MatrixXd &_sc1,
                                                         MatrixXd &_sc2) {
    ...
    const int SEARCH_RADIUS =
            round(0.5 * SEARCH_RATIO * _sc1.cols()); // a half of search range
...
MatrixXd SCManager::makeScancontext(pcl::PointCloud<SemanticSurfel> &_scan_down) {
...
void SCManager::makeAndSaveScancontextAndKeys(
        pcl::PointCloud<SemanticSurfel> &_scan_down) {
...
std::pair<int, float> SCManager::detectLoopClosureID(void) {
    ...
    auto curr_key =
            polarcontext_invkeys_mat_.back();   // current observation (query)
    ...
    polarcontext_tree_->index->findNeighbors(
            knnsearch_result, &curr_key[0] /* query */, nanoflann::SearchParams(10));
...
float SCManager::getYawDiff() { return yaw_diff_rad; }
