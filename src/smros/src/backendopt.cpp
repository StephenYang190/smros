...
BackEndOpt::BackEndOpt() : pose_graph_(), initials_(), results_() {
...
bool BackEndOpt::addEdge(int from, int to, const Eigen::Matrix4d &measurement,
    ...
    NonlinearFactor::shared_ptr factor(
            new BetweenFactor<Pose3>(from, to, Pose3(measurement), odometry_model));
...
bool BackEndOpt::setInitialValues(int id,
    ...
    if (addPrior) {
        ...
        noiseModel::Diagonal::shared_ptr priorModel =
                noiseModel::Diagonal::Variances(diagonal);
...
bool BackEndOpt::updatePoses(
        std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>
        &pose) {
    for (const auto &pair: results_) {
        if (pair.key >= pose.size())
            continue;

        pose[pair.key] = pair.value.cast<Pose3>().matrix();
    }
