...
void SemanticMap::callBackPose(const nav_msgs::Odometry &msg) {
    tmp_pose_mutex_.lock();
    tmp_local_poses_.push(msg);
    tmp_pose_mutex_.unlock();
}

void SemanticMap::callBackPointCloud(const sensor_msgs::PointCloud2ConstPtr& msg) {
    tmp_point_cloud_mutex_.lock();
    tmp_point_cloud_.push(msg);
    tmp_point_cloud_mutex_.unlock();
}

void SemanticMap::callBackLoopClosure(const smros_msgs::Keyframe& msg) {
    ...
    if(odometry.header.seq == crt_index)
    {
        ...
        if(timestamp_ > 0)
        {
            pose_graph_.addEdge(timestamp_ - 1, timestamp_, local_pose.cast<double>());
        }
        timestamp_++;
    }
