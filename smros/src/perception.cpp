...
void Perception::PointCloudCallback(const sensor_msgs::PointCloud2 &msg) {
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    points_xyzi_list[iter + 3] = in_pointcloud_->points[i].data[3];
  }
  // get semantic result
  ...
  point2Surfel(semantic_label, msg.header.seq);
  generateNormal();
  publishRangeImage();
...
bool Perception::generateVertexMap(std::vector<int> &semantic_result) {
  ...
  for (int i = 0; i < num_points; i++) {
    ...
    vertex_maps_[u][v].point_x = in_pointcloud_->points[i].x;
  }
...
bool Perception::computeUVIndex(int point_index, int &u, int &v, float &r_xyz) {
  ...
  if (r_xyz < min_distance_ || r_xyz > max_distance_) {
    in_pointcloud_->points[point_index].y = NAN;
    return false;
  }
...
void Perception::point2Surfel(std::vector<int> &semantic_result,
                              int timestamp) {
  point_number_ = 0;
  ...
  for (int u = 0; u < width_; u++) {
    for (int v = 0; v < height_; v++) {
      ...
      int label = semantic_result[index];
      // set label
      net_->setColorMap(label);
      point.point_type = label;
      point.r = net_->getColorR() / 256.0f;
      point.g = net_->getColorG() / 256.0f;
      point.b = net_->getColorB() / 256.0f;
      point.create_timestamp = timestamp;
      point.update_timestamp = timestamp;
      point.confidence = initial_confidence_;
      point_number_++;
      label_numbers[point.point_type]++;
    }
  }
}
