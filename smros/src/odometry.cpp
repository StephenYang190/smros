...
Odometry::Odometry()
  ...
  point_cloud_sub_ =
      nh_.subscribe("surfel", 1000, &Odometry::PointCloudCallback, this);
}

void Odometry::PointCloudCallback(const sensor_msgs::PointCloud2& msg) {
  pcl::fromROSMsg(msg, *current_point_cloud_);
  if (init_) {
    current_pose =
        computePose(last_point_cloud_, current_point_cloud_, last_pose_);
    float distance = 0;
    for (int i = 0; i < 3; i++) {
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber point_cloud_sub =
      nh.subscribe("surfel", 1000, &Odometry::PointCloudCallback, &odometry);
  ros::Rate r(10);
  while (ros::ok()) {
    ros::spinOnce();
    r.sleep();
